...
{
    //int dnsService = 0;
    static readonly Country UNKNOWN_COUNTRY = new Country("--", "N/A");
    static int COUNTRY_BEGIN = 16776960;
    static int STRUCTURE_INFO_MAX_SIZE = 20;
    static int DATABASE_INFO_MAX_SIZE = 100;
    static int FULL_RECORD_LENGTH = 100; //???
    static int SEGMENT_RECORD_LENGTH = 3;
    static int STANDARD_RECORD_LENGTH = 3;
    static int ORG_RECORD_LENGTH = 4;
    static int MAX_RECORD_LENGTH = 4;
    static int MAX_ORG_RECORD_LENGTH = 1000; //???
    static int FIPS_RANGE = 360;
    static int STATE_BEGIN_REV0 = 16700000;
    static int STATE_BEGIN_REV1 = 16000000;
    static int US_OFFSET = 1;
    static int CANADA_OFFSET = 677;
    static int WORLD_OFFSET = 1353;
    ...
    static readonly String[] countryCode = {
    ...
    static readonly String[] countryName = {
    ...
    public LookupService(Stream database, int options) {
        try {
            file = database;
            dboptions = options;
            init();
    ...
    public LookupService(Stream database) : this(database, GEOIP_STANDARD) {}
    ...
    public int getID(long ipAddress) {
        if (file == null)
            throw new Exception("Database has been closed.");
        var ret = SeekCountry(ipAddress) - databaseSegments[0];
        return ret;
    }
            ...
            lock (this) {
                ...
                for (var i = 0; i < DATABASE_INFO_MAX_SIZE; i++) {
                    ...
                    if (delim[0] == 0 && delim[1] == 0 && delim[2] == 0) {
                        ...
                        databaseInfo = new DatabaseInfo(new String(dbInfo2));
                        return databaseInfo;
                    }
                    file.Seek(-4, SeekOrigin.Current);
                }
    ...
    public Location getLocation(long ipnum) {
        ...
        try {
            ...
            for (var a0 = 0; a0 < FULL_RECORD_LENGTH; a0++)
    ...
    public String getOrg(long ipnum) {
        ...
        int record_pointer;
        var str_length = 0;
        var buf = new byte[MAX_ORG_RECORD_LENGTH];
        var buf2 = new char[MAX_ORG_RECORD_LENGTH];
        String org_buf;
    ...
    int SeekCountry(long ipAddress) {
        ...
        for (var depth = 31; depth >= 0; depth--) {
            try {
                if ((dboptions & GEOIP_MEMORY_CACHE) == 1) {
                    for (var i = 0; i < (2*MAX_RECORD_LENGTH); i++)
